...
Core subsystems and external subsystems are created using the same code infrastructure: the [`SubsystemBuilder` object](../../source_docs/subsystem_builder.ipynb). This makes swapping out a core Aviary subsystem with an external one relatively straightforward, as they share the same interface.
