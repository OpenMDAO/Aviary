...
class AircraftModelReader(object):
    ...
    def read_case_recorder_file(self):
        ...
        if "final" not in cr.list_cases():
            raise ValueError(f"Case recorder file, {self._case_recorder_file} does not have expected case named 'final'")
    ...
    def get_variable_from_case(self, var_prom_name, units=None):
        ...
        raise ValueError(f"Promoted name {var_prom_name} not found")
    ...
    def get_variable_from_aviary_options(self, var_prom_name):
        ...
        value
            Value of the variable.
        """
        value, units = self.aviary_options.get_item(var_prom_name)
        return value
...
class Fuselage(object):
    ...
    def read_variables(self):
        try:
            self._length = self._reader.get_variable_from_case("aircraft:fuselage:length", units="ft")
            self._radius = self._reader.get_variable_from_case("aircraft:fuselage:avg_diameter", units="ft") / 2.0
...
class VerticalTail(object):
    ...
    def read_variables(self):
        ...
        """
        try:
            thickness_to_chord = self._reader.get_variable_from_case("aircraft:vertical_tail:thickness_to_chord")
            area = self._reader.get_variable_from_case("aircraft:vertical_tail:area", units="ft**2")
            aspect_ratio = self._reader.get_variable_from_case("aircraft:vertical_tail:aspect_ratio")
            self._taper_ratio = self._reader.get_variable_from_case("aircraft:vertical_tail:taper_ratio")
    ...
    def get_aframe_markup(self):
        ...
        left_quad = Quad3D(
            [
                ...
                Point3D(self._fuselage.radius, -self._fuselage.length/2., self._thickness/2),
                # point 1 - leading edge at root
                Point3D(self._fuselage.radius, -self._fuselage.length/2. + self._chord, self._thickness/2),
                # point 2 - leading edge at tip
                Point3D(self._span + self._fuselage.radius,-self._fuselage.length/2. + self._chord * self._taper_ratio, 
                        self._thickness/2),
                # point 3 - trailing edge at tip
                Point3D(self._span + self._fuselage.radius, -self._fuselage.length/2., self._thickness/2),
            ]
        )
...
class HorizontalWing(object):
    ...
    def read_variables(self):
        ...
        try:
            aspect_ratio = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:aspect_ratio")
            taper_ratio = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:taper_ratio")
            area = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:area", units="ft**2")
            try: 
                thickness_to_chord = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:thickness_to_chord")
            except ValueError:
                thickness_to_chord_root = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:thickness_to_chord_root")
                thickness_to_chord_tip = self._reader.get_variable_from_case(f"aircraft:{wing_type_name}:thickness_to_chord_tip")
                thickness_to_chord = (thickness_to_chord_root + thickness_to_chord_tip) / 2.0

            self._span = (aspect_ratio * area) ** 0.5
            self._chord = area / self._span
            self._thickness = thickness_to_chord * self._chord
            sweep_angle = 0.0  # TODO can we get this value?
            self._sweep_angle_tan = math.tan(math.radians(sweep_angle))
            self._chord_tip = self._chord * taper_ratio
            if self._wing_type == WingType.WING:
                mounting_type = self._reader.get_variable_from_case("aircraft:wing:mounting_type")
                self._vertical_position = 2.0 * (mounting_type-0.5) * self._fuselage.radius
            ...
            print(f"Warning: Unable to read horizontal wing of type '{self._wing_type}' variables due to the error: {e} ")
            raise
    ...
    def get_aframe_markup(self):
        ...
        left_top_quad = Quad3D(
            [
                ...
                Point3D(self._vertical_position + self._thickness/2, self._position_along_fuselage + self._chord/2., 0.0),
                # point 1 - leading edge at tip
                Point3D(self._vertical_position + self._thickness/2, self._position_along_fuselage + self._chord/2. - self._sweep_angle_tan * self._span/2., self._span/2.),
                # point 2 - trailing edge at tip
                Point3D(self._vertical_position + self._thickness/2, self._position_along_fuselage + self._chord/2. - self._sweep_angle_tan * self._span/2. - self._chord_tip, 
                        self._span/2.),
...
class Engines(object):
    ...
    """
    def __init__(self, reader, wing):
        self._reader = reader
        self.wing_span = wing.span
    ...
    def read_variables(self):
        ...
        """
        try:
            self._num_wing_engines = self._reader.get_variable_from_aviary_options(av.Aircraft.Engine.NUM_WING_ENGINES)
            self._engine_diameter = self._reader.get_variable_from_case(av.Aircraft.Nacelle.AVG_DIAMETER, units="ft")
            self._engine_length = self._reader.get_variable_from_case(av.Aircraft.Nacelle.AVG_LENGTH, units="ft")
            self._engine_locations_on_wing = self._reader.get_variable_from_aviary_options(av.Aircraft.Engine.WING_LOCATIONS)
            try:
                self._has_propellers = self._reader.get_variable_from_case(av.Aircraft.Engine.HAS_PROPELLERS)
            except ValueError as e2:
                self._has_propellers = False
            self._has_propellers = True  # TODO remove this!
        except Exception as e:
    ...
    def get_aframe_markup(self):
        ...
        for engine_location in self._engine_locations_on_wing:
            distance_from_fuselage = engine_location * self.wing_span 
            entities += f'''
                    <!-- engine -->
                    <a-cylinder id="cylinder" position="0 0 {distance_from_fuselage}" radius="{self._engine_diameter/2}" height="{self._engine_length}" 
                        rotation="0 90 0" color="white"></a-cylinder>
                    <a-cylinder id="cylinder" position="0 0 {-distance_from_fuselage}" radius="{self._engine_diameter/2}" height="{self._engine_length}" 
                        rotation="0 90 0" color="white"></a-cylinder>
            '''
            if self._has_propellers:
                propeller_disc_height = self._engine_length / 10.
                propeller_disc_radius = self._engine_diameter/2 * 1.2
                entities += f'''
                        <!-- engine -->
                        <a-cylinder id="cylinder" position="0 {self._engine_length/2} {distance_from_fuselage}" radius="{propeller_disc_radius}" height="{propeller_disc_height}" 
                            rotation="0 90 0" color="grey"></a-cylinder>
                        <a-cylinder id="cylinder" position="0 {self._engine_length/2} {-distance_from_fuselage}" radius="{propeller_disc_radius}" height="{propeller_disc_height}" 
                            rotation="0 90 0" color="grey"></a-cylinder>
                '''
...
class Aircraft3DModel(object):
    ...
    def read_variables(self):
        ...
        self.engines = Engines(self._reader, self.wing)
