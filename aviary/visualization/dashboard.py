...
def create_csv_frame(csv_filepath, documentation):
    ...
    documentation : str
        Explaination of what this tab is showing.
    ...
    if os.path.exists(csv_filepath):
        ...
        report_pane = pn.Column(
            pn.pane.HTML(f"<p>{documentation}</p>", styles = {'text-align': documentation_text_align}),
            df_pane
        )
    else:
        report_pane = None
...
def dashboard(script_name, problem_recorder, driver_recorder, port):
    ...
    # Inputs
    inputs_pane = create_report_frame("html", f"{reports_dir}/inputs.html", "Detailed report on the model inputs")
    if inputs_pane:
    ...
    input_list_pane = create_report_frame("text", "input_list.txt", '''
       A plain text display of the model inputs. Recommended for beginners. Only created if debug_mode is set to True in the input deck.
        ...
        different phases and within different components. Note that its values can be different because its value has 
    ...
    snopt_summary_pane = create_report_frame("text", f"{reports_dir}/SNOPT_summary.out", '''
        This report is generated by the SNOPT optimizer that summarizes the optimization results
                                             ''')
    ...
    for md_file in Path(f"{reports_dir}subsystems").glob("*.md"):
        subsystems_pane = create_report_frame("markdown", str(md_file), f"A {md_file.stem} subsystem report")
