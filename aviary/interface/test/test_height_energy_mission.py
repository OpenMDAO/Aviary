...
class AircraftMissionTestSuite():
    ...
    def xxxnorunmission_basic_and_dashboard(self):
    ...
    def xxxnorunmission_basic_pyopt(self):
        prob = self.run_mission(self.phase_info, 'IPOPT')
        self.assertIsNotNone(prob)
        self.assertTrue(prob.problem_ran_successfully)

    @require_pyoptsparse(optimizer='IPOPT')
    def xxxnorunmission_optimize_mach_only(self):
    ...
    @require_pyoptsparse(optimizer='IPOPT')
    def xxxnorunmission_optimize_altitude_and_mach(self):
    ...
    @require_pyoptsparse(optimizer='IPOPT')
    def xxxnorunmission_altitude_optimize_only(self):
        # Test with altitude_optimize flag set to True
        modified_phase_info = self.phase_info.copy()
        for phase in ['climb', 'cruise', 'descent']:
            modified_phase_info[phase]['user_options']['altitude_optimize'] = True
    ...
    @require_pyoptsparse(optimizer='IPOPT')
    def xxxnorunmission_distance_solve_segments_IPOPT(self):
        modified_phase_info = self.phase_info.copy()
        for phase in ['climb', 'cruise', 'descent']:
    ...
    def xxxnorunmission_distance_solve_segments_SLSQP(self):
        modified_phase_info = self.phase_info.copy()
        for phase in ['climb', 'cruise', 'descent']:
            modified_phase_info[phase]['user_options']['distance_solve_segments'] = True
    ...
    @require_pyoptsparse(optimizer='IPOPT')
    def xxxnorunmission_with_external_subsystem(self):
    ...
    def xxxnoruncustom_phase_builder(self):
    ...
    def xxxnoruncustom_phase_builder_error(self):
        local_phase_info = self.phase_info.copy()
    ...
    def xxxnorunsupport_constraint_aliases(self):
    ...
    def xxxnoruntrajectory_warning(self):
        modified_phase_info = deepcopy(self.phase_info)
