...
class AircraftMissionTestSuite(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i, cruise_dict in enumerate(cruise_dicts):
            cruise_dict["user_options"]["initial_bounds"] = (
                (64.0 + i*10, 192.0 + i*10), "min")
            cruise_dict["user_options"]["duration_bounds"] = (
                (56.5 + i*10, 169.5 + i*10), "min")
    ...
    def test_linkages(self):
        ...
        for phase_1, var, phase_2 in linkage_data:
            ...
            self.assertIn(linkage_string, constraints,
                          f"Linkage {linkage_string} is missing")

        for phase_1, var, phase_2 in not_linkage_data:
            linkage_string = get_linkage_string(phase_1, var, phase_2)
            self.assertNotIn(linkage_string, constraints,
                             f"Linkage {linkage_string} is present")
