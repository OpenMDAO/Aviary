...
from aviary.interface.build_model import _exec_build_model, _setup_build_model_parser
...
_command_map = {
    ...
    'build_model': (_setup_build_model_parser, _exec_build_model,
                    'Build or edit an aircraft model.'),
}
...
def aviary_cmd():
    ...
    if not set(args).intersection(subs.choices) and len(
            args) == 1 and os.path.isfile(cmdargs[0]):
