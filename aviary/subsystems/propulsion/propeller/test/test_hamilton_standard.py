...
class PreHamiltonStandardTest(unittest.TestCase):
    ...
    def test_preHS(self):
        ...
        tol = 5e-4
        assert_near_equal(
            prob.get_val('power_coefficient'), [0.3871, 0.3147, 0.2815], tolerance=tol
        )
        assert_near_equal(
...
class HamiltonStandardTest(unittest.TestCase):
    ...
    def test_HS(self):
        ...
        prob.set_val(Dynamic.Atmosphere.MACH, [0.001509, 0.1887, 0.4976], units='unitless')
        ...
        tol = 5e-4
        assert_near_equal(
            prob.get_val('thrust_coefficient'), [0.2763, 0.2052, 0.1158], tolerance=tol
        )
...
class PostHamiltonStandardTest(unittest.TestCase):
    ...
    def test_postHS(self):
        ...
        prob.set_val(
            Dynamic.Vehicle.Propulsion.PROPELLER_TIP_SPEED, [700.0, 750.0, 800.0], units='ft/s'
        )
        prob.set_val(
            Dynamic.Atmosphere.DENSITY,
        ...
        tol = 5e-4
        assert_near_equal(
            prob.get_val('thrust_coefficient_comp_loss'), [0.2765, 0.2052, 0.1137], tolerance=tol
        )
        assert_near_equal(
        ...
        assert_near_equal(
            prob.get_val('propeller_efficiency'), [0.321, 0.2735, 0.1588], tolerance=tol
        )
        assert_near_equal(
            prob.get_val('install_efficiency'), [0.3167, 0.2680, 0.15378], tolerance=tol
        )
