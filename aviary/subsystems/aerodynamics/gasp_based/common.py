...
class AeroForces(om.ExplicitComponent):
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            ...
            [Dynamic.Vehicle.LIFT_COEFFICIENT, Dynamic.Atmosphere.DYNAMIC_PRESSURE],
            rows=arange,
            cols=arange,
        )
        ...
        self.declare_partials(
            ...
            [Dynamic.Vehicle.DRAG_COEFFICIENT, Dynamic.Atmosphere.DYNAMIC_PRESSURE],
            rows=arange,
            cols=arange,
        )
    ...
    def compute_partials(self, inputs, J):
        ...
        J[Dynamic.Vehicle.LIFT, Dynamic.Vehicle.LIFT_COEFFICIENT] = q * wing_area
        J[Dynamic.Vehicle.LIFT, Dynamic.Atmosphere.DYNAMIC_PRESSURE] = CL * wing_area
        J[Dynamic.Vehicle.LIFT, Aircraft.Wing.AREA] = q * CL

        J[Dynamic.Vehicle.DRAG, Dynamic.Vehicle.DRAG_COEFFICIENT] = q * wing_area
...
class CLFromLift(om.ExplicitComponent):
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            Dynamic.Vehicle.LIFT_COEFFICIENT,
            ['lift_req', Dynamic.Atmosphere.DYNAMIC_PRESSURE],
            rows=ar,
            cols=ar,
        )
    ...
    def compute(self, inputs, outputs):
        ...
        outputs[Dynamic.Vehicle.LIFT_COEFFICIENT] = lift_req / (q * wing_area)
...
class TimeRamp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('y', shape=(k, nn), units=units, desc='Input values ramped over time')
...
class TanhRampComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        for output_name, options in self._ramps.items():
            ...
            self.add_output(output_name, shape=(nn,) + options['shape'], units=options['units'])

            self.add_input(
                options['initial_val_name'],
                val=options['initial_val'] * np.ones(options['shape']),
                units=options['units'],
            )
            self.add_input(
                options['final_val_name'],
                val=options['final_val'] * np.ones(options['shape']),
                units=options['units'],
            )
            self.add_input(
            ...
            self.add_input(
                options['t_duration_name'],
                val=options['t_duration_val'],
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        for name, options in self._ramps.items():
            ...
            dtanh_term_dtduration = omtanh2 * (-2 * np.pi * (t - t_init) / t_duration**2)
