...
class WingFuselageInterferencePremission(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            'top_and_bottom_width',
            TopAndBottomWidth(),
            promotes_inputs=['*'],
            promotes_outputs=['*'],
        )
        self.add_subsystem(
            'body_ratios', BodyRatios(), promotes_inputs=['*'], promotes_outputs=['*']
        )
        self.add_subsystem(
            'interference_drag',
            InterferenceDrag(),
...
class WingFuselageInterferenceMission(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # log10(10_000_000.) = 7
        CFIN = 0.455 / 7**2.58 / (1.0 + 0.144 * EM**2) ** 0.65  # dynamic
        CDWI = FCFWC * FCFWT * CFIN  # dynamic
        FEW_over_SW = CDWI * CKW * ((np.log10(RELI * CBARW) / 7.0) ** (-2.6))  # dynamic
        # from interference.f
