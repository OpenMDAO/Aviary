...
class GroundEffect(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Dynamic.ALTITUDE, np.zeros(nn), units='m')

        add_aviary_input(self, Dynamic.FLIGHT_PATH_ANGLE, val=np.zeros(nn), units='rad')

        self.add_input(
            'minimum_drag_coefficient', 0.0,
            desc='coefficient of minimum drag', units='unitless'
        )

        self.add_input(
            'base_lift_coefficient', val=np.ones(nn),
            desc='coefficient of lift without ground effect', units='unitless'
        )

        self.add_input(
            'base_drag_coefficient', val=np.ones(nn),
        ...
        self.add_output(
            ...
            desc='coefficient of lift with ground effect', units='unitless'
        )

        self.add_output(
            'drag_coefficient', val=np.ones(nn),
            desc='coefficient of drag with ground effect', units='unitless'
        )
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            'lift_coefficient', [Dynamic.ALTITUDE, 'base_lift_coefficient'],
        ...
        self.declare_partials(
            'lift_coefficient',
            [
                'angle_of_attack', Dynamic.FLIGHT_PATH_ANGLE, 'minimum_drag_coefficient',
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        altitude = inputs[Dynamic.ALTITUDE]
        ...
        height_factor[idx] = 10. * (1.1 - ground_effect_state[idx])

        aspect_ratio_term = (6. + aspect_ratio)**2 / (36. + aspect_ratio)
        ground_effect_term0 = 32. * (ground_effect_state * aspect_ratio_term)**2 + 1.

        lift_coeff_factor_denom = (
            ground_effect_term0 - 0.5 + 4. * ground_effect_state * aspect_ratio_term
            * np.sqrt(ground_effect_term0)
        )

        lift_coeff_factor = 1. + height_factor / lift_coeff_factor_denom

        lift_coefficient = base_lift_coefficient * lift_coeff_factor

        combined_angle = angle_of_attack + flight_path_angle
        ground_effect_term1 = 1. + 32. * ground_effect_state**2

        drag_coeff_factor_denom = \
            4. * ground_effect_state * np.sqrt(ground_effect_term1) + ground_effect_term1

        drag_coeff_factor = 1. - height_factor / drag_coeff_factor_denom

        drag_coefficient = (
            minimum_drag_coefficient
            + lift_coeff_factor * drag_coeff_factor
            * (base_drag_coefficient - minimum_drag_coefficient)
            + combined_angle * base_lift_coefficient * (lift_coeff_factor - 1.)
        )
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        idx = np.where(ground_effect_state > 1.0)
        height_factor[idx] = 10. * (1.1 - ground_effect_state[idx])
        d_hf_alt[idx] = -10. / span

        aspect_ratio_term = (6. + aspect_ratio)**2 / (36. + aspect_ratio)
        ground_effect_term0 = 32. * (ground_effect_state * aspect_ratio_term)**2 + 1.

        d_get0_alt = (64. * ground_effect_state * aspect_ratio_term**2) * d_ges_alt

        sqrt_get0 = np.sqrt(ground_effect_term0)

        lift_coeff_factor_denom = (
            ground_effect_term0 - 0.5
            + 4. * ground_effect_state * aspect_ratio_term * sqrt_get0
        )
        ...
        d_lcfd_alt = (
            ...
            + 4. * aspect_ratio_term * (
                d_ges_alt * sqrt_get0
                + ground_effect_state * (0.5 / sqrt_get0) * d_get0_alt
            )
        )
        ...
        J['lift_coefficient', Dynamic.ALTITUDE] = base_lift_coefficient * d_lcf_alt
        ...
        f = (6. + aspect_ratio)**2
        d_f = 2. * (6. + aspect_ratio)
        g = (36. + aspect_ratio)
        d_g = 1.

        d_art_ar = ((d_f * g) - (f * d_g)) / g**2

        d_get0_ar = 64. * (ground_effect_state**2 * aspect_ratio_term) * d_art_ar

        d_lcfd_ar = d_get0_ar + 4. * ground_effect_state * (
            d_art_ar * sqrt_get0
            + aspect_ratio_term * 0.5 / sqrt_get0 * d_get0_ar
        )
        ...
        J['lift_coefficient', Aircraft.Wing.ASPECT_RATIO] = \
            base_lift_coefficient * d_lcf_ar
        ...
        d_hf_b[idx] = -10. * d_ges_b[idx]

        d_get0_b = 64. * (ground_effect_state * aspect_ratio_term**2) * d_ges_b

        d_lcfd_b = (
            d_get0_b + 4. * aspect_ratio_term * (
                (d_ges_b * sqrt_get0)
                + (ground_effect_state * 0.5 / sqrt_get0 * d_get0_b)
            )
        )
        ...
        drag_coeff_factor_denom = \
            4. * ground_effect_state * sqrt_get1 + ground_effect_term1

        drag_coeff_factor = 1. - height_factor / drag_coeff_factor_denom

        d_dc_aoa = base_lift_coefficient * (lift_coeff_factor - 1.) * d_ca_aoa

        J['drag_coefficient', 'angle_of_attack'] = d_dc_aoa
        # endregion drag_coefficient wrt angle_of_attack

        # region drag_coefficient wrt flight_path_angle
        d_ca_fpa = 1.

        d_dc_fpa = base_lift_coefficient * (lift_coeff_factor - 1.) * d_ca_fpa

        J['drag_coefficient', Dynamic.FLIGHT_PATH_ANGLE] = d_dc_fpa
        # endregion drag_coefficient wrt flight_path_angle

        # region drag_coefficient wrt altitude
        d_get1_alt = 64. * ground_effect_state * d_ges_alt
        ...
        d_dcfd_alt = 4. * ((d_f * g) + (f * d_g)) + d_get1_alt
        ...
        d_dc_alt = (
            ((d_f * g) + (d_g * f)) * (base_drag_coefficient - minimum_drag_coefficient)
            + combined_angle * base_lift_coefficient * d_lcf_alt
        )

        J['drag_coefficient', Dynamic.ALTITUDE] = d_dc_alt
        # endregion drag_coefficient wrt altitude

        # region drag_coefficient wrt minimum_drag_coefficient
        d_dc_mdc = 1. - lift_coeff_factor * drag_coeff_factor
        ...
        d_dc_blc = combined_angle * (lift_coeff_factor - 1.)
        ...
        d_get1_b = 64. * ground_effect_state * d_ges_b
        ...
        d_dcfd_b = 4. * ((d_f * g) + (f * d_g)) + d_get1_b
        ...
        d_dc_b = (
            ((d_f * g) + (f * d_g)) * (base_drag_coefficient - minimum_drag_coefficient)
        ...
        idx = np.where(ground_effect_state > 1.1)
        if idx:
            J['drag_coefficient', Dynamic.ALTITUDE][idx] = 0.0
            ...
            J['drag_coefficient', Dynamic.FLIGHT_PATH_ANGLE][idx] = 0.0

            J['lift_coefficient', Dynamic.ALTITUDE][idx] = 0.0
