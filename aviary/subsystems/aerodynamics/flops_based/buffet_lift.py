...
class BuffetLift(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # Declare outputs
        self.add_output(
            'DELCLB', shape=nn, units='unitless', desc='Delta lift coefficient before buffet onset'
        )
    ...
    def compute_partials(self, inputs, partials):
        ...
        # wrt SW25
        dCLB_dSW25 = (1.0 / a) * AR * FCLB * (1.0 + CAM / 10.0) * np.sin(SW25_rad) * cos_fact**2

        # wrt CAM
        dCLB_dCAM = FCLB * AR / 10.0 * cos_fact

        partials['DELCLB', Dynamic.Atmosphere.MACH] = dCLB_dMach
        partials['DELCLB', Aircraft.Design.MACH] = dCLB_ddesign_Mach
...
BUFTTable = InterpND(
    method='lagrange2', points=(BUFT[1:, 0], BUFT[0, 1:]), values=BUFT[1:, 1:], extrapolate=True
)
