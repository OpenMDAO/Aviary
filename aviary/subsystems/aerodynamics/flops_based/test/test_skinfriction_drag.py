...
class SkinFrictionDragTest(unittest.TestCase):
    ...
    def test_derivs_multiengine(self):
        ...
        fine = np.array([0.13, .125, .1195, 10.0392, 1.5491,
                        1.5491, 1.125, 1.125, 1.125, 1.125])
        Re = np.array([[15.0, 11, 18.3, 183.4, 17.6, 17.6, 5.55, 5.55, 5.55, 5.55],
                      [17.0, 16, 15.3, 78.4, 23.6, 33.6, 22.7, 22.7, 22.7, 22.7]]) * 1e-6
        cf = np.array([[.00263, .00276, .00255, .00182, .00256, .00256, 0.0184, 0.0184, 0.0184, 0.0184],
                       [.00283, .00296, .00235, .00172, .00276, .00276, 0.0203, 0.0203, 0.0203, 0.0203]])
        Sw = np.array([2396.56, 592.65, 581.13, 4158.62, 273.45,
                      273.45, 239.1, 239.1, 239.1, 239.1])
        lam_up = np.array([6 * 0.03])
        lam_low = np.array([6 * 0.02])
        nc = len(fine)
