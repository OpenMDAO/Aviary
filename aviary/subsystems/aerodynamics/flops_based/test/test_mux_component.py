...
class TestMuxComponent(unittest.TestCase):
    ...
    def test_mux_multiengine(self):
        ...
        prob.set_val(Aircraft.Nacelle.WETTED_AREA, np.array([42.0, 33.5]))
        prob.set_val(Aircraft.Nacelle.FINENESS, np.array([43.0, 37.0]))
        prob.set_val(Aircraft.Nacelle.CHARACTERISTIC_LENGTH, np.array([44.0, 30.0]))
        prob.set_val(Aircraft.Nacelle.LAMINAR_FLOW_UPPER, np.array([0.09, 0.13]))
