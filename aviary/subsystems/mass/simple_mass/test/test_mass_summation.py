...
class MassSummationTest(unittest.TestCase):
    ...
    def test_case(self):
        ...
        self.prob.model.set_input_defaults(Aircraft.Wing.SPAN, val=1.0, units='m')

        self.prob.model.set_input_defaults(Aircraft.Wing.ROOT_CHORD, val=1.0, units='m')
        ...
        self.prob.model.set_input_defaults('tip_chord', val=0.5, units='m')

        self.prob.model.set_input_defaults('tip_chord_tail', val=0.5, units='m')

        self.prob.model.set_input_defaults('thickness_ratio', val=0.12)

        self.prob.model.set_input_defaults('skin_thickness', val=0.002, units='m')

        self.prob.model.set_input_defaults('twist', val=jnp.zeros(10), units='deg')

        self.prob.model.set_input_defaults('twist_tail', val=jnp.zeros(10), units='deg')

        self.prob.model.set_input_defaults(Aircraft.Fuselage.LENGTH, val=2.5, units='m')

        self.prob.model.set_input_defaults('base_diameter', val=0.4, units='m')

        self.prob.model.set_input_defaults('tip_diameter', val=0.2)
        ...
        self.prob.model.set_input_defaults('thickness', val=0.05, units='m')
        ...
        thickness_dist = (
            5
            * max_thickness_chord_ratio
            * (0.2969 * jnp.sqrt(x) - 0.1260 * x - 0.3516 * x**2 + 0.2843 * x**3 - 0.1015 * x**4)
        )

        self.prob.model.set_input_defaults('thickness_dist', val=thickness_dist, units='m')
        ...
        if self.prob.model.tot.tail_mass.options['tail_type'] == 'horizontal':
            assert_near_equal(self.prob['structure_mass'], 355.18828485, tol)
        else:
            assert_near_equal(self.prob['structure_mass'], 355.18828485, tol)
...
class StructureMassTest(unittest.TestCase):
    ...
    def test_case(self):
        ...
        if self.prob.model.tot.options['tail_type'] == 'horizontal':
            assert_near_equal(self.prob['structure_mass'], 108.45, tol)
        else:
            assert_near_equal(self.prob['structure_mass'], 108.45, tol)
