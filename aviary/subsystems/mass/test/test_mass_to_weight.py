...
class MassToWeightTest(unittest.TestCase):
    ...
    def test_case(self):
        ...
        assert_near_equal(prob.get_val('weight', 'lbf'), 120_000 * GRAV_ENGLISH_LBM, 1.0e-12)
        ...
        prob.model.add_subsystem(
            'calc_weight', MassToWeight(num_nodes=nn), promotes=['mass', 'weight']
        )
        ...
        prob.model.list_inputs()
        prob.model.list_outputs()
        assert_near_equal(prob.get_val('weight', 'lbf'), mass * GRAV_ENGLISH_LBM, 1.0e-12)
        partial_data = prob.check_partials(compact_print=True, method='cs')
        assert_check_partials(partial_data, atol=1e-10, rtol=1e-12)
