...
class WingBendingMass(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        ...
        fact3 = 1.0 - 0.1 * faert
        W1NIR = self.A1 * bt * fact1 * ulf * span * fact2 * fact3 * cayf * vfact * pctl * 1.0e-6
        ...
        dW1NIR_faert = (
            -self.A1 * bt * fact1 * ulf * span * fact2 * 0.1 * cayf * vfact * pctl * 1.0e-6
        )
        dW1NIR_varswp = (
            self.A1 * bt * fact1 * ulf * span * fact2 * fact3 * cayf * dvfact_varswp * pctl * 1.0e-6
        )
        dW1NIR_sweep = (
            self.A1 * bt * fact1 * ulf * span * fact2 * fact3 * cayf * dvfact_sweep * pctl * 1.0e-6
        )
