...
class BodyTankCalculations(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_output(self, Aircraft.Fuel.TOTAL_CAPACITY, val=0)
    ...
    def compute(self, inputs, outputs):
        ...
        if smooth:
            extra_fuel_wt = extra_fuel_wt * sigX(extra_fuel_wt / 50)
        else:
            if extra_fuel_wt < 0:
                extra_fuel_wt = 0
        max_extra_fuel_wt = extra_fuel_volume * rho_fuel
        wingfuel_wt_min = fuel_wt_min - max_extra_fuel_wt
        wingfuel_wt_min = wingfuel_wt_min * sigX(wingfuel_wt_min)
        max_fuel_avail_est = fuel_wt_des + extra_fuel_wt
        max_fuel_avail_new = gross_wt_initial - OEW
        est_GTOW = OEW + max_fuel_avail_est
        max_fuel_avail = max_fuel_avail_est * sigX(
            (gross_wt_initial - est_GTOW) / 110
        ) + max_fuel_avail_new * sigX((est_GTOW - gross_wt_initial) / 110)
    ...
    def compute_partials(self, inputs, J):
        ...
        if smooth:
            dExtraFuelWt_dReqFuelWt = (
                sigX((req_fuel_wt - max_wingfuel_wt) / 50)
                + (req_fuel_wt - max_wingfuel_wt)
                * dSigXdX((req_fuel_wt - max_wingfuel_wt) / 50)
                * 1
                / 50
            )
            dExtraFuelWt_dMaxWingfuelWt = -1 * sigX(
                (req_fuel_wt - max_wingfuel_wt) / 50
            ) + (req_fuel_wt - max_wingfuel_wt) * dSigXdX(
                (req_fuel_wt - max_wingfuel_wt) / 50
            ) * (
                -1 / 50
            )
        ...
        J["extra_fuel_volume", Aircraft.Fuel.WING_VOLUME_DESIGN] = dSigXdX(
            design_fuel_vol - max_wingfuel_vol
        ) * (design_fuel_vol - geometric_fuel_vol) + sigX(
            design_fuel_vol - max_wingfuel_vol
        )
        J["extra_fuel_volume", Aircraft.Fuel.WING_VOLUME_STRUCTURAL_MAX] = (
            dSigXdX(design_fuel_vol - max_wingfuel_vol)
            * (-1)
            * (design_fuel_vol - geometric_fuel_vol)
        )
        J["extra_fuel_volume", Aircraft.Fuel.WING_VOLUME_GEOMETRIC_MAX] = -sigX(
            design_fuel_vol - max_wingfuel_vol
        )
        ...
        dMEFWt_dDesFuelVol = (
            dSigXdX(design_fuel_vol - max_wingfuel_vol)
            * (design_fuel_vol - geometric_fuel_vol)
            + sigX(design_fuel_vol - max_wingfuel_vol)
        ) * rho_fuel
        ...
        dMEFWt_dGeomFuelVol = (
            -sigX(design_fuel_vol - max_wingfuel_vol) * rho_fuel
        )
        ...
        J["wingfuel_mass_min", "fuel_mass_min"] = sigX(fuel_wt_min - max_extra_fuel_wt) + (
            fuel_wt_min - max_extra_fuel_wt
        ) * dSigXdX(fuel_wt_min - max_extra_fuel_wt)
        J["wingfuel_mass_min", Aircraft.Fuel.WING_VOLUME_DESIGN] = ((
            fuel_wt_min - max_extra_fuel_wt
        ) * dSigXdX(fuel_wt_min - max_extra_fuel_wt) * (
            -dMEFWt_dDesFuelVol
        ) - dMEFWt_dDesFuelVol * sigX(
            fuel_wt_min - max_extra_fuel_wt
        )) / GRAV_ENGLISH_LBM
        J["wingfuel_mass_min", Aircraft.Fuel.WING_VOLUME_STRUCTURAL_MAX] = (fuel_wt_min * dSigXdX(
            fuel_wt_min - max_extra_fuel_wt
        ) * (-dMEFWt_dMaxWingfuelVol) - (
            dMEFWt_dMaxWingfuelVol * sigX(fuel_wt_min - max_extra_fuel_wt)
            + max_extra_fuel_wt
            * dSigXdX(fuel_wt_min - max_extra_fuel_wt)
            * (-dMEFWt_dMaxWingfuelVol)
        )) / GRAV_ENGLISH_LBM
        J["wingfuel_mass_min", Aircraft.Fuel.WING_VOLUME_GEOMETRIC_MAX] = (fuel_wt_min * dSigXdX(
            fuel_wt_min - max_extra_fuel_wt
        ) * (-dMEFWt_dGeomFuelVol) - (
            dMEFWt_dGeomFuelVol * sigX(fuel_wt_min - max_extra_fuel_wt)
            + max_extra_fuel_wt
            * dSigXdX(fuel_wt_min - max_extra_fuel_wt)
            * (-dMEFWt_dGeomFuelVol)
        )) / GRAV_ENGLISH_LBM
        J["wingfuel_mass_min", Aircraft.Fuel.DENSITY] = fuel_wt_min * dSigXdX(
            fuel_wt_min - max_extra_fuel_wt
        ) * (-dMEFWt_dRhoFuel) - (
            dMEFWt_dRhoFuel * sigX(fuel_wt_min - max_extra_fuel_wt)
            + max_extra_fuel_wt
            * dSigXdX(fuel_wt_min - max_extra_fuel_wt)
            * (-dMEFWt_dRhoFuel)
        )

        int1 = sigX((gross_wt_initial - est_GTOW) / 110)
        int2 = sigX((est_GTOW - gross_wt_initial) / 110)
        max_fuel_avail = max_fuel_avail_est * int1 + max_fuel_avail_new * int2

        dInt1_dFuelWtDes = dSigXdX((gross_wt_initial - est_GTOW) / 110) * (-1 / 110)
        dInt1_dReqFuelWt = dSigXdX((gross_wt_initial - est_GTOW) / 110) * (
            -dExtraFuelWt_dReqFuelWt
        )
        dInt1_dMaxWingfuelWt = dSigXdX((gross_wt_initial - est_GTOW) / 110) * (
            -dExtraFuelWt_dMaxWingfuelWt
        )

        dInt2_dOEM = dSigXdX((est_GTOW - gross_wt_initial) / 110) / 110
        dInt2_dFuelWtDes = dSigXdX((est_GTOW - gross_wt_initial) / 110) / 110
        dInt2_dReqFuelWt = (
            dSigXdX((est_GTOW - gross_wt_initial) / 110) * dExtraFuelWt_dReqFuelWt
        )
        dInt2_dMaxWingfuelWt = (
            dSigXdX((est_GTOW - gross_wt_initial) / 110) * dExtraFuelWt_dMaxWingfuelWt
        )
        dInt2_dGTOW = dSigXdX((est_GTOW - gross_wt_initial) / 110) * (-1 / 110)
...
class FuelAndOEMOutputs(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Aircraft.Fuel.DENSITY, val=6.687, units="lbm/ft**3")
        add_aviary_input(self, Mission.Design.GROSS_MASS, val=175400)
        add_aviary_input(self, Aircraft.Propulsion.MASS, val=16129)
        add_aviary_input(self, Aircraft.Controls.TOTAL_MASS, val=3895.0)
        ...
        add_aviary_output(self, Aircraft.Fuel.WING_VOLUME_DESIGN, val=0)
        add_aviary_output(self, Aircraft.Design.OPERATING_MASS, val=0)
        ...
        add_aviary_output(self, Aircraft.Fuel.WING_VOLUME_STRUCTURAL_MAX, val=0)
    ...
    def compute(self, inputs, outputs):
        ...
        max_wingfuel_wt = OEM_wingfuel_wt * sigX(
            (volume_wingfuel_wt - OEM_wingfuel_wt) / 95
        ) + volume_wingfuel_wt * sigX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
    ...
    def compute_partials(self, inputs, J):
        ...
        dMaxWFWt_dGTOW = (
            OEM_wingfuel_wt
            * dSigXdX((volume_wingfuel_wt - OEM_wingfuel_wt) / 95)
            * dOEMwingfuelWt_dGTOW
            + dOEMwingfuelWt_dGTOW *
            sigX((volume_wingfuel_wt - OEM_wingfuel_wt) / 95)
            + volume_wingfuel_wt
            * dSigXdX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
            * dOEMwingfuelWt_dGTOW
        )
        dMaxWFWt_dPropWt = (
            OEM_wingfuel_wt
        ...
        dMaxWFWt_dGeomFuelVol = (
            ...
            * dSigXdX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
            * rho_fuel
            + rho_fuel * sigX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
        )
        dMaxWFWt_dRhoFuel = (
            OEM_wingfuel_wt
            * dSigXdX((volume_wingfuel_wt - OEM_wingfuel_wt) / 95)
            ...
            * dSigXdX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
            * geometric_fuel_vol
            + geometric_fuel_vol * sigX((OEM_wingfuel_wt - volume_wingfuel_wt) / 95)
        )
...
class FuelSysAndFullFuselageMass(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Aircraft.Wing.MASS, val=15830.0)
        self.add_input(
            "wing_mounted_mass",
        ...
        add_aviary_input(self, Aircraft.Fuel.FUEL_SYSTEM_MASS_SCALER, val=1)
        add_aviary_input(self, Aircraft.Fuel.FUEL_SYSTEM_MASS_COEFFICIENT, val=0.041)
        add_aviary_input(self, Aircraft.Fuel.DENSITY, val=6.687, units="lbm/galUS")
        add_aviary_input(self, Mission.Design.FUEL_MASS, val=3000)
        add_aviary_input(self, Aircraft.Fuel.FUEL_MARGIN, val=10)
        self.add_input(
            "wingfuel_mass_min",
...
class FuselageAndStructMass(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Aircraft.Wing.MASS_SCALER, val=1)
        add_aviary_input(self, Aircraft.Wing.MASS, val=5000)
        add_aviary_input(self, Aircraft.HorizontalTail.MASS_SCALER, val=1)
        add_aviary_input(self, Aircraft.HorizontalTail.MASS, val=2275)
        add_aviary_input(self, Aircraft.VerticalTail.MASS_SCALER, val=1)
        add_aviary_input(self, Aircraft.VerticalTail.MASS, val=2297)
        add_aviary_input(self, Aircraft.Fuselage.MASS_SCALER, val=1)
        ...
        add_aviary_input(self, Aircraft.Engine.POD_MASS_SCALER, val=np.ones(count))
        ...
        add_aviary_output(self, Aircraft.Design.STRUCTURE_MASS, val=0)
        add_aviary_output(self, Aircraft.Fuselage.MASS, val=0)
...
class FuelMass(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Aircraft.Design.STRUCTURE_MASS, val=50461.0)
        add_aviary_input(self, Aircraft.Fuel.FUEL_SYSTEM_MASS, val=1759,)
        add_aviary_input(self, Mission.Design.GROSS_MASS, val=175400)
        self.add_input(
            "eng_comb_mass",
            val=14371.0,
        ...
        add_aviary_input(self, Aircraft.Design.FIXED_USEFUL_LOAD, val=4932.0)
        self.add_input(
            "payload_mass_des", val=36000, units="lbm", desc="WPLDES: design payload"
        )
        add_aviary_input(self, Aircraft.Fuel.FUEL_SYSTEM_MASS_SCALER, val=1)
        add_aviary_input(self, Aircraft.Fuel.FUEL_SYSTEM_MASS_COEFFICIENT, val=0.041)
        add_aviary_input(self, Aircraft.Fuel.DENSITY, val=6.687, units="lbm/galUS")
        ...
        add_aviary_output(self, Mission.Design.FUEL_MASS, val=0)
        add_aviary_output(self, Aircraft.Propulsion.MASS, val=0)
        add_aviary_output(self, Mission.Design.FUEL_MASS_REQUIRED, val=0)
        self.add_output(
            "fuel_mass_min",
            val=0,
            units="lbm",
...
class FuelMassGroup(om.Group):
    ...
    def setup(self):
        ...
        self.set_input_defaults(Aircraft.Wing.MASS, val=15830, units="lbm")
        ...
        self.set_input_defaults(Aircraft.Controls.TOTAL_MASS, val=3895, units="lbm")
        self.set_input_defaults(
            Aircraft.Design.FIXED_EQUIPMENT_MASS, val=21089.0, units="lbm")
        self.set_input_defaults(Aircraft.Design.FIXED_USEFUL_LOAD, val=4932, units="lbm")
