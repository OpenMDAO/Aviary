...
class TailTestCase(unittest.TestCase):  # this is the large single aisle 1 V3 test case
    ...
    def test_case1(self):
        ...
        tol = 5e-4
        assert_near_equal(
            self.prob["loc_MAC_vtail"], 0.44959578484694906, tol
        )  # bug fixed value
        # bug fixed value
        assert_near_equal(self.prob[Aircraft.HorizontalTail.MASS], 2285, tol)
        assert_near_equal(self.prob[Aircraft.VerticalTail.MASS],
                          2312, tol)  # bug fixed value
...
class HighLiftTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.prob.model.set_input_defaults(
            Aircraft.Wing.AREA, val=1764.6, units="ft**2"
        )
        self.prob.model.set_input_defaults(
        ...
        self.prob.model.set_input_defaults(
            Aircraft.Wing.SLAT_SPAN_RATIO, val=0.9, units="unitless"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Wing.FLAP_SPAN_RATIO, val=0.88, units="unitless"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Wing.LOADING, val=93.1, units="lbf/ft**2"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Wing.THICKNESS_TO_CHORD_ROOT, val=0.11, units="unitless"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Wing.SPAN, val=185.8, units="ft"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Fuselage.AVG_DIAMETER, val=13.1, units="ft"
        )
        self.prob.model.set_input_defaults(
            Aircraft.Wing.CENTER_CHORD, val=13.979, units="ft"
        )
        self.prob.model.set_input_defaults(
...
class FixedMassGroupTestCase1(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        assert_near_equal(
            self.prob["c_strut_braced"], 1, tol
        )  # bug fixed value and original value
        assert_near_equal(
            self.prob["c_gear_loc"], 1, tol
        )  # bug fixed value and original value
        assert_near_equal(
            self.prob[Aircraft.Engine.POSITION_FACTOR], 0.95, tol
        )  # bug fixed value and original value
...
class FixedMassGroupTestCase2(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        assert_near_equal(
            self.prob[Aircraft.Wing.MATERIAL_FACTOR], 1.2213063198183813, tol)
        assert_near_equal(
            self.prob["c_strut_braced"], 0.9928, tol
        )  # not actual GASP value
        ...
        assert_near_equal(
            self.prob["tail.loc_MAC_vtail"], 1.799, tol
        )  # not actual GASP value
        # original GASP value
        assert_near_equal(self.prob[Aircraft.HorizontalTail.MASS], 2275, tol)
        assert_near_equal(self.prob[Aircraft.VerticalTail.MASS],
                          2297, tol)  # original GASP value
        # original GASP value
        ...
        assert_near_equal(
            self.prob["eng_comb_mass"], 14599.28196478, tol
        )  # not actual GASP value
        assert_near_equal(
            self.prob["wing_mounted_mass"], 24027.6, tol
        )  # not actual GASP value
        assert_near_equal(
            self.prob["engine.prop_mass_all"], 0, tol
        )  # bug fixed and original value

        partial_data = self.prob.check_partials(
            out_stream=None, method="cs", form="central",
        )
                    ...
                    for gear_loc in [0.0, 0.05, 0.01]:
                        for fuse_mounted in [False, True]:
                            for num_engines in [2, 4]:
                                ...
                                data.set_val(
                                    Aircraft.LandingGear.MAIN_GEAR_LOCATION, gear_loc)
                                data.set_val(
                                    Aircraft.Wing.FLAP_TYPE, flap_type)
                                data.set_val(
                                    Aircraft.Electrical.HAS_HYBRID_SYSTEM, has_hybrid)
                                data.set_val(
