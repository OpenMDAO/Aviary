...
class PreMissionGroupTest(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        assert_near_equal(self.prob[Aircraft.Design.FIXED_USEFUL_LOAD], 5176.4317956, tol)
        ...
        assert_near_equal(prob[Aircraft.Fuel.FUEL_SYSTEM_MASS], 1747.1907, tol)
        assert_near_equal(prob[Aircraft.Design.STRUCTURE_MASS], 50656.4801, tol)
        assert_near_equal(prob[Aircraft.Fuselage.MASS], 18662.0778, tol)
        assert_near_equal(prob[Mission.Design.FUEL_MASS_REQUIRED], 42614.4079, tol)
        assert_near_equal(prob[Aircraft.Propulsion.MASS], 16054.9326, tol)
        assert_near_equal(prob[Mission.Design.FUEL_MASS], 42614.4079, tol)
        assert_near_equal(prob[Aircraft.Fuel.WING_VOLUME_DESIGN], 851.9092, tol)
        assert_near_equal(prob[Aircraft.Design.OPERATING_MASS], 96785.592, tol)
    ...
    def test_case2(self):
        ...
        assert_near_equal(prob[Aircraft.LandingGear.MAIN_GEAR_MASS], 6366.3591, tol)
        assert_near_equal(self.prob[Aircraft.Design.FIXED_EQUIPMENT_MASS], 21078.3911, tol)
        assert_near_equal(self.prob[Aircraft.Design.FIXED_USEFUL_LOAD], 5167.6838, tol)
        ...
        assert_near_equal(prob[Aircraft.Fuel.FUEL_SYSTEM_MASS], 1786.0962, tol)
        assert_near_equal(prob[Aircraft.Design.STRUCTURE_MASS], 50070.2644776, tol)
        assert_near_equal(prob[Aircraft.Fuselage.MASS], 18664.08560432, tol)
        assert_near_equal(prob[Mission.Design.FUEL_MASS_REQUIRED], 43563.3227, tol)
        assert_near_equal(prob[Aircraft.Propulsion.MASS], 15700.9815, tol)
        assert_near_equal(prob[Mission.Design.FUEL_MASS], 43563.3227, tol)
        assert_near_equal(prob[Aircraft.Fuel.WING_VOLUME_DESIGN], 870.879, tol)
        assert_near_equal(prob[Aircraft.Design.OPERATING_MASS], 95836.6773, tol)
        assert_near_equal(prob[Aircraft.Fuel.AUXILIARY_FUEL_CAPACITY], 0, tol)
