...
class TestSubsystemBuilderBase(unittest.TestCase):
    ...
    def test_get_states(self):
        ...
        for key, value in states.items():
            self.assertIsInstance(
                value, dict, msg=f"the value for '{key}' should be a dictionary")

            self.assertIsInstance(
                value['rate_source'], str, msg=f"the value for 'rate_source' key in '{key}' should be a string")
    ...
    def test_get_bus_variables(self):
        ...
        for name, values in bus_variables.items():
            ...
            self.assertIn('mission_name', values.keys(),
    ...
    def test_build_mission(self):
        ...
        mission_sys = self.subsystem_builder.build_mission(
            10, aviary_inputs=self.aviary_values)
        if mission_sys is not None:
            self.assertIsInstance(mission_sys, System,
                                  "The method should return an OpenMDAO System object.")
    ...
    def test_get_design_vars(self):
        ...
        self.assertIsInstance(
            design_vars, dict, "get_design_vars() should return a dictionary")

        # Verify that the keys in the dictionary are strings
        for key in design_vars.keys():
            self.assertIsInstance(
                key, str, "The keys in the dictionary returned by get_design_vars() should be strings")

        # Verify that the values in the dictionary are also dictionaries
        for val in design_vars.values():
            self.assertIsInstance(
                val, dict, "The values in the dictionary returned by get_design_vars() should be dictionaries")

        # Verify that the dictionaries have the correct keys
        for val in design_vars.values():
            self.assertIn(
                'units', val, "The dictionaries returned by get_design_vars() should have a 'units' key")
            self.assertIn(
                'lower', val, "The dictionaries returned by get_design_vars() should have a 'lower' key")
            self.assertIn(
    ...
    def test_get_parameters(self):
        ...
        for key in parameters.keys():
            self.assertIsInstance(
                key, str, "The keys in the dictionary returned by get_parameters() should be strings")
    ...
    def test_get_initial_guesses(self):
        ...
        self.assertIsInstance(initial_guesses, dict,
                              msg="get_initial_guesses() should return a dictionary")
        for key, val in initial_guesses.items():
            self.assertIsInstance(
                val, dict, msg=f"The value for '{key}' should be a dictionary")
            self.assertIn(
            ...
            guess_val = val['val']
            self.assertIsInstance(guess_val, (float, list, np.ndarray),
    ...
    def test_get_mass_names(self):
        ...
        self.assertIsInstance(subsystem_builder.get_mass_names(), list,
        ...
        for mass_name in subsystem_builder.get_mass_names():
            self.assertIsInstance(mass_name, str,
                                  "Each mass name in the list should be a string")

    def test_preprocess_inputs(self):
        if not hasattr(self, 'aviary_values'):
            self.aviary_values = AviaryValues()
        inputs = self.subsystem_builder.preprocess_inputs(self.aviary_values)
        self.assertIsInstance(inputs, AviaryValues,
                              "preprocess_inputs did not return an AviaryValues object")
    ...
    def test_check_pre_mission(self):
        ...
        self.assertIsInstance(pre_mission_sys, System,
                              "The method should return an OpenMDAO System object.")
    ...
    def test_check_design_variables(self):
        ...
        for key, value in design_vars.items():
            ...
            self.assertTrue(design_var_exists,
                            f"Design variable '{key}' not found in the model.")
    ...
    def test_check_initial_guesses(self):
        ...
        for key, value in initial_guesses.items():
            ...
            self.assertTrue(initial_guess_exists,
                            f"Initial guess '{key}' not found in the model.")
