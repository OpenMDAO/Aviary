...
class GlideTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.prob.model.set_input_defaults(
            "mass", np.ones(num_nodes)*165279, units="lbm")
        self.prob.model.set_input_defaults(
            Aircraft.Wing.AREA, np.ones(num_nodes)*1370.3, units="ft**2")
        self.prob.model.set_input_defaults(
            Mission.Landing.GLIDE_TO_STALL_RATIO, np.ones(num_nodes)*1.3, units="unitless")
        self.prob.model.set_input_defaults(
            "CL_max", np.ones(num_nodes)*2.9533, units="unitless")
        self.prob.model.set_input_defaults(
    ...
    def test_case1(self):
        ...
        assert_near_equal(self.prob.get_val("TAS_touchdown", units="kn"),
                          np.ones(num_nodes)*126.3081, tol
                          )  # note: actual GASP value is: 126.27
        assert_near_equal(self.prob.get_val("density_ratio", units="unitless"),
                          np.ones(num_nodes)*1.0, tol
                          )  # note: calculated from GASP glide speed values as: .998739
        assert_near_equal(self.prob.get_val("wing_loading_land", units="lbf/ft**2"),
                          np.ones(num_nodes)*120.61519375, tol
                          )  # note: actual GASP value is: 120.61
        assert_near_equal(self.prob.get_val("theta", units="deg"),
                          np.ones(num_nodes)*3.56857, tol
                          )  # note: actual GASP value is: 3.57
        assert_near_equal(self.prob.get_val("glide_distance", units="ft"),
                          np.ones(num_nodes)*801.7444, tol
                          )  # note: actual GASP value is: 802
        assert_near_equal(self.prob.get_val("tr_distance", units="ft"),
                          np.ones(num_nodes)*166.5303, tol
                          )  # note: actual GASP value is: 167
        assert_near_equal(self.prob.get_val("delay_distance", units="ft"),
                          np.ones(num_nodes)*213.184, tol
                          )  # note: actual GASP value is: 213
        assert_near_equal(self.prob.get_val("flare_alt", units="ft"),
                          np.ones(num_nodes)*20.73407, tol
...
class GroundRollTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.prob.model.set_input_defaults(
            "touchdown_CD", val=np.ones(num_nodes)*0.07344)
        self.prob.model.set_input_defaults(
            "touchdown_CL", val=np.ones(num_nodes)*1.18694)
        self.prob.model.set_input_defaults(
        ...
        self.prob.model.set_input_defaults(
            "TAS_touchdown", val=np.ones(num_nodes)*126.27, units="kn")
        self.prob.model.set_input_defaults(
            "thrust_idle", val=np.ones(num_nodes)*1276, units="lbf")
        self.prob.model.set_input_defaults(
        ...
        self.prob.model.set_input_defaults(
            "glide_distance", val=np.ones(num_nodes)*802, units="ft")
        self.prob.model.set_input_defaults(
            "tr_distance", val=np.ones(num_nodes)*167, units="ft")
        self.prob.model.set_input_defaults(
            "delay_distance", val=np.ones(num_nodes)*213, units="ft")
        self.prob.model.set_input_defaults(
            "CL_max", np.ones(num_nodes)*2.9533, units="unitless")
        self.prob.model.set_input_defaults(
            "mass", np.ones(num_nodes)*165279, units="lbm")
    ...
    def test_case1(self):
        ...
        assert_near_equal(self.prob[Mission.Landing.GROUND_DISTANCE],
                          np.ones(num_nodes)*3588.43116212, tol
