...
varinfo = {
    ...
    Dynamic.Mission.DISTANCE: (Dynamic.Mission.RANGE, "states:distance", "NM"),
...
def setup_climb1():
    ...
    preprocess_propulsion(option_defaults, [engine])

    ode_args["aviary_options"] = option_defaults
    ...
    climb1.add_objective("time", loc="final", ref=1000.)
    # climb1.add_design_var("rhs_all.throttle", lower=0., upper=1.)
    ...
    prob.set_val(
        "climb1.states:altitude", climb1.interp(
            Dynamic.Mission.ALTITUDE, ys=[
                500, 10000]))
    # prob.set_val(
    #     "climb1.rhs_all.throttle", climb1.interp(
    #         Dynamic.Mission.ALTITUDE, ys=[
    #             0.40, 1.0]))
