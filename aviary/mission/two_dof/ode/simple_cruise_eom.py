...
class DistanceComp(om.ExplicitComponent):
    ...
    def setup_partials(self):
        ...
        self.declare_partials('cruise_range', 'cruise_time', rows=row_col, cols=row_col)
        self.declare_partials('cruise_range', 'TAS_cruise', rows=row_col, cols=row_col)
        self.declare_partials('cruise_range', 'cruise_distance_initial', rows=0, cols=0, val=1.0)

    def compute(self, inputs, outputs):
        v_x = inputs['TAS_cruise']
        r0 = inputs['cruise_distance_initial']
        t = inputs['cruise_time']
        t0 = t[0]

        outputs['cruise_range'][0] = r0
        outputs['cruise_range'][1:] = r0 + v_x * (t - t0)
