...
class StallSpeed(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            'lift_coefficient_max', val=1., units='unitless',
            desc='maximum lift coefficient')

        self.add_output(
            'stall_speed', val=np.zeros(nn), units='m/s',
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        stall_speed = (2. * weight / (density * area * lift_coefficient_max))**0.5
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        J['stall_speed', 'lift_coefficient_max'] = \
...
class TakeoffEOM(om.Group):
    ...
    def setup(self):
        ...
        inputs = [Dynamic.Vehicle.FLIGHT_PATH_ANGLE, Dynamic.Atmosphere.VELOCITY]
        outputs = [Dynamic.Mission.DISTANCE_RATE, Dynamic.Atmosphere.ALTITUDE_RATE]
        ...
        self.add_subsystem(
            'accelerations', Accelerations(num_nodes=nn),
            promotes_inputs=['*'],
            promotes_outputs=['*']
        )

        self.add_subsystem(
            'velocity_rate', VelocityRate(num_nodes=nn),
            promotes_inputs=['*'],
            promotes_outputs=['*']
        )
...
class DistanceRates(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(
            self, Dynamic.Atmosphere.VELOCITY, val=np.zeros(nn), units='m/s'
        )
...
class Accelerations(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output(
            ...
            desc='current horizontal acceleration', units='m/s**2')
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            'acceleration_vertical', 'forces_horizontal', dependent=False)

        self.declare_partials(
            'acceleration_horizontal', 'forces_vertical', dependent=False)
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        J['acceleration_vertical', 'forces_vertical'] = 1. / mass
...
class VelocityRate(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_output(
            self, Dynamic.Atmosphere.VELOCITYITY_RATE, val=np.ones(nn), units='m/s**2'
        )
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        J[Dynamic.Atmosphere.VELOCITYITY_RATE, 'acceleration_horizontal'] = v_h / den
        J[Dynamic.Atmosphere.VELOCITYITY_RATE, 'acceleration_vertical'] = v_v / den

        J[Dynamic.Atmosphere.VELOCITYITY_RATE, Dynamic.Mission.DISTANCE_RATE] = (
...
class FlightPathAngleRate(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs[Dynamic.Vehicle.FLIGHT_PATH_ANGLE_RATE] = x
...
class SumForces(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        options.declare(
            ...
            desc='current friction coefficient, either rolling friction or braking'
            ' friction')
    ...
    def setup_partials(self):
        ...
        if climbing:
            self.declare_partials(
                'forces_horizontal', Dynamic.Vehicle.MASS, dependent=False
            )
            ...
            self.declare_partials(
                'forces_horizontal', Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL, val=val, rows=rows_cols,
                cols=rows_cols)
            ...
            self.declare_partials(
                'forces_horizontal',
                ['angle_of_attack', Dynamic.Vehicle.FLIGHT_PATH_ANGLE],
                dependent=False,
            )
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        J['forces_horizontal', Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL] = c_angle
        J['forces_vertical', Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL] = s_angle
        ...
        J['forces_horizontal', Dynamic.Vehicle.FLIGHT_PATH_ANGLE] = (
            -thrust * s_angle + drag * s_gamma - lift * c_gamma
        )

        J['forces_vertical', Dynamic.Vehicle.FLIGHT_PATH_ANGLE] = (
...
class ClimbGradientForces(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Dynamic.Vehicle.LIFT, val=np.ones(nn), units='N')
        add_aviary_input(
            self, Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL, val=np.ones(nn), units='N')
        add_aviary_input(self, Dynamic.Vehicle.DRAG, val=np.ones(nn), units='N')
        ...
        self.add_output(
            ...
            desc='current sum of forces in the horizontal direction; checking for excess'
            ' thrust')

        self.add_output(
            'climb_gradient_forces_vertical', val=np.zeros(nn), units='N',
    ...
    def compute_partials(self, inputs, J, discrete_inputs=None):
        ...
        J[f_h_key, Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL] = c_angle
        J[f_v_key, Dynamic.Vehicle.Propulsion.THRUSTsion.THRUST_TOTAL] = s_angle

        J[f_h_key, 'angle_of_attack'] = -thrust * s_angle
        J[f_v_key, 'angle_of_attack'] = thrust * c_angle

        J[f_h_key, Dynamic.Vehicle.FLIGHT_PATH_ANGLE] = -weight * c_gamma
        J[f_v_key, Dynamic.Vehicle.FLIGHT_PATH_ANGLE] = weight * s_gamma
