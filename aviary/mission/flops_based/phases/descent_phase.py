...
Descent = None  # forward declaration for type hints


@register
class Descent(EnergyPhase):
    '''
    Define a phase builder class for a typical FLOPS based descent phase.
    ...
    user_options : AviaryValues (<empty>)
        ...
        supported options:
            ...
            - required_altitude_rate : float (None)
                minimum avaliable descent rate
            - no_descent : bool (False)
                aircraft is not allowed to descent during phase
            - no_descent : bool (False)
