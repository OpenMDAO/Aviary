...
class NPSSExternalCodeComp(om.ExternalCodeComp):
    ...
    def setup(self):
        ...
        self.add_output(
            'Fn_SLS', val=1.0, units='lbf', desc='net thrust at sea-level-static conditions'
        )
        self.add_output(
            'Wf_training_data', val=np.ones(vec_size), units='lbm/s', desc='fuel flow training data'
        )
        self.add_output(
            'thrust_training_data', val=np.ones(vec_size), units='lbf', desc='thrust training data'
        )
        self.add_output(
            'thrustmax_training_data',
    ...
    def compute(self, inputs, outputs):
        ...
        with open(self.input_file, 'w') as input_file:
            input_file.write(
                'Alt_DES  = %.16f ;\nMN_DES  = %.16f ;\nW_DES  = %.16f ;' % (Alt_DES, MN_DES, W_DES)
            )
