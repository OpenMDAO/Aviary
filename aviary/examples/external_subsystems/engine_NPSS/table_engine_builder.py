...
class TableEngineBuilder(EngineModel):
    ...
    def __init__(self, name='NPSS_prop_system'):
    ...
    def build_mission(self, num_nodes, aviary_inputs):
        ...
        csv_path = pkg_resources.resource_filename(
            "aviary",
            os.path.join("examples", "external_subsystems", "engine_NPSS", "NPSS_Model", "Output",  "RefEngine.outputAviary")
        )

        engine_data = np.genfromtxt(csv_path, skip_header=0)

        # Sort the data by Mach, then altitude, then throttle
        engine_data = engine_data[np.lexsort((engine_data[:,2], engine_data[:,1],engine_data[:,0]))]
        ...
        for i in range(engine_data.shape[0]):
            ...
            index = np.where((engine_data[:,0] == engine_data[i,0]) & (engine_data[:,1] == engine_data[i,1]) & (engine_data[:,2] == 1.0))[0][0]
    ...
    def get_design_vars(self):
        ...
        '''
        mass_flow_dict = {'units': 'lbm/s', 'upper': 450, 'lower': 100, 'ref': 450}  # upper and lower are just notional for now
