...
class BatteryPreMission(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        add_aviary_input(self, Aircraft.Battery.ENERGY_REQUIRED, meta_data=ExtendedMetaData)
        ...
        # params (fixed)
        add_aviary_input(self, Aircraft.Battery.Cell.HEAT_CAPACITY, meta_data=ExtendedMetaData)
        add_aviary_input(self, Aircraft.Battery.Case.HEAT_CAPACITY, meta_data=ExtendedMetaData)
        add_aviary_input(self, Aircraft.Battery.Cell.MASS, meta_data=ExtendedMetaData)
        add_aviary_input(self, Aircraft.Battery.Cell.VOLTAGE_LOW, meta_data=ExtendedMetaData)
        add_aviary_input(self, Aircraft.Battery.Cell.DISCHARGE_RATE, meta_data=ExtendedMetaData)
        add_aviary_input(
            self, Aircraft.Battery.Cell.ENERGY_CAPACITY_MAX, meta_data=ExtendedMetaData
        )
        add_aviary_input(self, Aircraft.Battery.Case.WEIGHT_FRAC, meta_data=ExtendedMetaData)
    ...
    def compute(self, inputs, outputs):
        ...
        outputs[Aircraft.Battery.N_PARALLEL] = (
            max_amps / inputs[Aircraft.Battery.Cell.DISCHARGE_RATE]
        )
        ...
        outputs[Aircraft.Battery.HEAT_CAPACITY] = (
            inputs[Aircraft.Battery.Cell.HEAT_CAPACITY]
            + inputs[Aircraft.Battery.Case.HEAT_CAPACITY]
            * (1.0 - inputs[Aircraft.Battery.Case.WEIGHT_FRAC])
        ) / inputs[Aircraft.Battery.Case.WEIGHT_FRAC]
        outputs[Aircraft.Battery.VOLUME] = (
            outputs[Aircraft.Battery.N_SERIES]
            * outputs[Aircraft.Battery.N_PARALLEL]
            * inputs[Aircraft.Battery.Cell.VOLUME]
        )
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[Aircraft.Battery.N_SERIES, Aircraft.Battery.Cell.VOLTAGE_LOW] = -bus_v / (c_l_v**2)
        # n_{parallel}
        partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.CURRENT_MAX] = 1.0 / I_rate
        partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.Cell.DISCHARGE_RATE] = -max_amps / (
            I_rate**2
        )
        # mass_{battery}
        partials[Aircraft.Battery.MASS, Aircraft.Battery.VOLTAGE] = (
            partials[Aircraft.Battery.N_SERIES, Aircraft.Battery.VOLTAGE] * n_p * m_cell * wf_case
        )
        partials[Aircraft.Battery.MASS, Aircraft.Battery.Cell.VOLTAGE_LOW] = (
            partials[Aircraft.Battery.N_SERIES, Aircraft.Battery.Cell.VOLTAGE_LOW]
            * n_p
            * m_cell
            * wf_case
        )
        partials[Aircraft.Battery.MASS, Aircraft.Battery.CURRENT_MAX] = (
            partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.CURRENT_MAX]
            * n_s
            * m_cell
            * wf_case
        )
        partials[Aircraft.Battery.MASS, Aircraft.Battery.Cell.DISCHARGE_RATE] = (
            partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.Cell.DISCHARGE_RATE]
            * n_s
            * m_cell
            * wf_case
        )
        partials[Aircraft.Battery.MASS, Aircraft.Battery.Cell.MASS] = n_s * n_p * wf_case
        partials[Aircraft.Battery.MASS, Aircraft.Battery.Case.WEIGHT_FRAC] = n_s * n_p * m_cell
        # C_{p,batt}
        partials[Aircraft.Battery.HEAT_CAPACITY, Aircraft.Battery.Cell.HEAT_CAPACITY] = (
            1.0 / wf_case
        )
        partials[Aircraft.Battery.HEAT_CAPACITY, Aircraft.Battery.Case.HEAT_CAPACITY] = (
            1.0 - wf_case
        ) / wf_case
        ...
        partials[Aircraft.Battery.VOLUME, Aircraft.Battery.Cell.VOLTAGE_LOW] = (
            partials[Aircraft.Battery.N_SERIES, Aircraft.Battery.Cell.VOLTAGE_LOW] * n_p * vc
        )
        partials[Aircraft.Battery.VOLUME, Aircraft.Battery.CURRENT_MAX] = (
            partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.CURRENT_MAX] * n_s * vc
        )
        partials[Aircraft.Battery.VOLUME, Aircraft.Battery.Cell.DISCHARGE_RATE] = (
            partials[Aircraft.Battery.N_PARALLEL, Aircraft.Battery.Cell.DISCHARGE_RATE] * n_s * vc
        )
