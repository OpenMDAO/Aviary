...
inputs.set_val(Aircraft.CrewPayload.Design.NUM_ECONOMY_CLASS, 158)
...
inputs.set_val(Aircraft.CrewPayload.NUM_ECONOMY_CLASS, 158)
...
inputs.set_val(Aircraft.Fuel.MAX_CAPACITY_MASS, 45694.0, 'lbm')
...
# Nacelle
# ---------------------------
inputs.set_val(Aircraft.Nacelle.REFERENCE_AVG_DIAMETER, 7.94, 'ft')
...
inputs.set_val(Aircraft.Wing.ASPECT_RATIO_REFERENCE, 11.22091)
inputs.set_val(Aircraft.Wing.BENDING_MATERIAL_MASS_SCALER, 1.0)
inputs.set_val(Aircraft.Wing.CHORD_PER_SEMISPAN_DISTRIBUTION, np.array([0.31, 0.23, 0.084]))
...
inputs.set_val(Aircraft.Wing.INPUT_STATION_DISTRIBUTION, np.array([0.0, 0.2759, 0.9367]))
inputs.set_val(Aircraft.Wing.LOAD_DISTRIBUTION_CONTROL, 2.0)
inputs.set_val(Aircraft.Wing.LOAD_FRACTION, 1.0)
inputs.set_val(Aircraft.Wing.LOAD_PATH_SWEEP_DISTRIBUTION, np.array([0.0, 22.0]), 'deg')
...
inputs.set_val(Aircraft.Wing.THICKNESS_TO_CHORD_REFERENCE, 0.1300)
inputs.set_val(Aircraft.Wing.THICKNESS_TO_CHORD_DISTRIBUTION, np.array([0.145, 0.115, 0.104]))
...
outputs.set_val(Mission.OPERATING_ITEMS_MASS, 5969.278, 'lbm')
...
outputs.set_val(Aircraft.CrewPayload.CABIN_CREW_MASS, 465.0, 'lbm')
outputs.set_val(Aircraft.CrewPayload.PASSENGER_SERVICE_MASS, 3022.74805809, 'lbm')
outputs.set_val(Aircraft.CrewPayload.PASSENGER_MASS_TOTAL, 30420.0, 'lbm')
...
outputs.set_val(Aircraft.LandingGear.TOTAL_MASS, 8780.91, 'lbm')
...
outputs.set_val(Aircraft.Nacelle.AVG_DIAMETER, np.array([7.94]), 'ft')
outputs.set_val(Aircraft.Nacelle.AVG_LENGTH, np.array([12.3]), 'ft')
