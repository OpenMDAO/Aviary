...
class TestOffDesign(TwoDOFTestCase):
    ...
    def test_off_design_IPOPT(self):
        ...
        prob_off_design_min_fuel.run_aviary_problem()

        off_design_max_range_range = prob_off_design_max_range.get_val(av.Mission.RANGE)
        off_design_min_fuel_mass = prob_off_design_min_fuel.get_val(av.Mission.GROSS_MASS)
        assert_near_equal(off_design_max_range_range, self.sized_range, tolerance=0.02)
        assert_near_equal(off_design_min_fuel_mass, self.sized_mass, tolerance=0.02)
    ...
    def test_off_design_SNOPT(self):
        ...
        prob_off_design_min_fuel.check_and_preprocess_inputs()
        prob_off_design_min_fuel.build_model()
        prob_off_design_min_fuel.add_driver('SNOPT', max_iter=100)
        prob_off_design_min_fuel.add_design_variables()
        prob_off_design_min_fuel.add_objective()
        prob_off_design_min_fuel.setup()
        prob_off_design_min_fuel.run_aviary_problem()
