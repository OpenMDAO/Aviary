...
class Battery(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials(
            ['voltage_out', 'power'], [Dynamic.Vehicle.Propulsion.CURRENT], rows=ar, cols=ar
        )
...
class ElectronicSpeedController(om.ExplicitComponent):
    def initialize(self):
        ...
        self.options.declare(
            'n',
            default=1.6054,
            desc='a coefficient for efficiency(throttle) equation: efficiency = a * (1 - 1 / (1 + c*throttle^d))',
        )
        self.options.declare(
            'o',
            default=1.6519,
            desc='b coefficient for efficiency(throttle) equation: efficiency = a * (1 - 1 / (1 + c*throttle^d))',
        )
        self.options.declare(
            'p',
            default=0.6455,
            desc='c coefficient for efficiency(throttle) equation: efficiency = a * (1 - 1 / (1 + c*throttle^d))',
        )
    ...
    def setup(self):
        ...
        self.declare_partials(
            'voltage_out', ['voltage_in', Dynamic.Vehicle.Propulsion.THROTTLE], rows=ar, cols=ar
        )
        self.declare_partials(
            'power',
            ['voltage_in', Dynamic.Vehicle.Propulsion.CURRENT, Dynamic.Vehicle.Propulsion.THROTTLE],
            rows=ar,
            cols=ar,
        )
        self.declare_partials(
            'current_out', Dynamic.Vehicle.Propulsion.CURRENT, val=1.0, rows=ar, cols=ar
        )
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['efficiency'] = np.where(
            thr >= transition,
            a * (1 - 1 / (1 + b * thr**c)),
            m * thr + ((a * (1 - 1 / (1 + b * transition**c))) - m * transition),
        )
        outputs['voltage_out'] = (
            inputs['voltage_in']
        ...
        outputs['power'] = (
            ...
            * inputs[Dynamic.Vehicle.Propulsion.CURRENT]
            * inputs['voltage_in']
        )
    ...
    def compute_partials(self, inputs, partials):
        ...
        efficiency = np.where(
            t >= transition,
            a * (1 - 1 / (1 + b * t**c)),
            m * t + ((a * (1 - 1 / (1 + b * transition**c))) - m * transition),
        )

        t_safe = np.where(t > 0, t, transition)
        partials['efficiency', Dynamic.Vehicle.Propulsion.THROTTLE] = np.where(
            t >= transition, a * b * c * t_safe ** (c - 1) / (b * t_safe**c + 1) ** 2, m
        )

        # partials['']
        partials['voltage_out', 'voltage_in'] = (
            inputs[Dynamic.Vehicle.Propulsion.THROTTLE] * efficiency
        )
        partials['voltage_out', Dynamic.Vehicle.Propulsion.THROTTLE] = inputs['voltage_in'] * (
            efficiency
            + inputs[Dynamic.Vehicle.Propulsion.THROTTLE]
            * partials['efficiency', Dynamic.Vehicle.Propulsion.THROTTLE]
        )

        partials['power', 'voltage_in'] = (efficiency - 1) * inputs[
            Dynamic.Vehicle.Propulsion.CURRENT
        ]
        partials['power', Dynamic.Vehicle.Propulsion.CURRENT] = (efficiency - 1) * inputs[
            'voltage_in'
        ]
        partials['power', Dynamic.Vehicle.Propulsion.THROTTLE] = (
            inputs[Dynamic.Vehicle.Propulsion.CURRENT]
            * inputs['voltage_in']
            * partials['efficiency', Dynamic.Vehicle.Propulsion.THROTTLE]
        )
...
class Motor(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials(
            ['power'], [Aircraft.Engine.Motor.RESISTANCE, Aircraft.Engine.Motor.IDLE_CURRENT]
        )
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['power'] = (
            -(inputs['current'] ** 2) * R
            - inputs[Aircraft.Engine.Motor.IDLE_CURRENT] * voltage_prop
        )
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[Dynamic.Vehicle.Propulsion.RPM, 'voltage_in'] = (
            lf * inputs[Aircraft.Engine.Motor.KV] * dvoltage_prop_dvoltage_in
        )
        partials[Dynamic.Vehicle.Propulsion.RPM, 'current'] = (
            lf * inputs[Aircraft.Engine.Motor.KV] * dvoltage_prop_dcurrent
        )
        partials[Dynamic.Vehicle.Propulsion.RPM, Aircraft.Engine.Motor.RESISTANCE] = (
            lf * inputs[Aircraft.Engine.Motor.KV] * dvoltage_prop_dresistance
        )
        partials[Dynamic.Vehicle.Propulsion.RPM, Aircraft.Engine.Motor.KV] = lf * voltage_prop

        partials['power', 'voltage_in'] = (
            -inputs[Aircraft.Engine.Motor.IDLE_CURRENT] * dvoltage_prop_dvoltage_in
        )
        partials['power', 'current'] = (
            -2 * inputs['current'] * R
            - inputs[Aircraft.Engine.Motor.IDLE_CURRENT] * dvoltage_prop_dcurrent
        )
        partials['power', Aircraft.Engine.Motor.RESISTANCE] = (
            -(inputs['current'] ** 2)
            - inputs[Aircraft.Engine.Motor.IDLE_CURRENT] * dvoltage_prop_dresistance
        )
...
class PropCoefficients(om.MetaModelSemiStructuredComp):
    ...
    def setup(self):
        ...
        self.add_input(
            ...
            training_data=xt[:, 1],
            units='inch',
            desc='propeller pitch',
        )
        self.add_input(
            Dynamic.Vehicle.Propulsion.RPM, val=np.zeros(nn), training_data=xt[:, 2], units='rev/s'
        )
        self.add_input(
            Dynamic.Mission.VELOCITY, val=np.zeros(nn), training_data=xt[:, 3], units='m/s'
        )
...
class Propeller(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials('rpm_constraint', Dynamic.Vehicle.Propulsion.RPM, rows=ar, cols=ar)
        self.declare_partials(
            'rpm_constraint',
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[Dynamic.Vehicle.Propulsion.THRUST, Dynamic.Atmosphere.DENSITY] = (
            n**2 * D**4 * inputs['ct'] * num_engines
        )
        partials[Dynamic.Vehicle.Propulsion.THRUST, Aircraft.Engine.Propeller.DIAMETER] = (
            rho * n**2 * 4 * D**3 * inputs['ct'] * num_engines
        )
        partials[Dynamic.Vehicle.Propulsion.THRUST, Dynamic.Vehicle.Propulsion.RPM] = (
            rho * 2 * n * D**4 * inputs['ct'] * num_engines
        )
