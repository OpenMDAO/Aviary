...
class MultiMissionProblem(om.Problem):
    ...
    # |||||||||||||||||||||| _add_guesses
                ...
                if 'mass' not in guesses:
                    ...
                    self.set_val(self.group_prefix+f'_{i}.traj.{phase_name}.states:mass',
    ...
    # def link_pre_post_traj(self):
    #     for i, prob in enumerate(self.probs):
    #         prefix = self.group_prefix+f'_{i}.'

    #         self.model.connect(prefix +
    #                            f'traj.{prob.regular_phases[-1]}.timeseries.distance',
    #                            Mission.Summary.RANGE, src_indices=[-1],
